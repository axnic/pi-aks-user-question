...
gh workflow run workflow_dispatch.release.yaml \
  --repo axnic/pi-aks-user-question \
...
```sh
gh run list --repo axnic/pi-aks-user-question --workflow=workflow_dispatch.release.yaml --limit=5
gh run view <run-id> --repo axnic/pi-aks-user-question
```
